...
def forward(model, train):
    ...
    for i in range(model.num_layers - 1):
        if i == 0:
            ...
            z_i = np.matmul(np.transpose(model.W[i]), list_a[i - 1]) + model.B[i]
            list_z.append(z_i)
            a_i = sigmoid(z_i)
            list_a.append(a_i)
...
def backward(model, train, y_real):
    y_hat, a_list = forward(model, train)
    deltas = []
    w_deriv = []
    b_deriv = []
    count = -1
